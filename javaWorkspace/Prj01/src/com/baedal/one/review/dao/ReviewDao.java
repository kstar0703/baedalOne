...
public class ReviewDao {
	...
	public ArrayList<ReviewReplyVo> userReview(Connection conn, String userNo) throws Exception {
		...
		// sql
		String sql = "SELECT MB.NICKNAME , REVIEW_RATING , R.USER_NO , TO_CHAR(R.WRITE_DATE,'YYYY-MM-DD hh24:mi') AS WRITE_DATE  , R.CONTENT  , TO_CHAR(RP.WRITE_DATE,'YYYY-MM-DD hh24:mi') AS REPLY_WRITE_DATE , RP.CONTENT REPLY_CONTENT , RP.REPLY_NO  , M.MENU_NAME  , R.ORDER_NO  , R.STORE_NO   , R.REVIEW_NO  , RP.DELETE_YN REPLY_DELETE_YN , S.STORE_NAME FROM REVIEW R   LEFT JOIN REPLY RP ON R.REVIEW_NO = RP.REVIEW_NO  JOIN ORDERS O ON R.ORDER_NO = O.ORDER_NO   JOIN MEMBER MB ON O.USER_NO = MB.MEMBER_NO   RIGHT JOIN CART_LIST C ON O.CART_NO = C.CART_NO   JOIN MENU M ON C.MENU_NO = M.MENU_NO   JOIN STORE S ON M.STORE_NO = S.STORE_NO WHERE O.USER_NO = ? AND R.DELETE_YN = 'N' ORDER BY R.REVIEW_NO";
		...
		while (rs.next()) {
			...
			ReviewVo reviewVo = new ReviewVo();
			reviewVo.setStoreName(rs.getString("STORE_NAME"));
