...
public class OrderDao {
	...
	public List<CartListDto> getCartList(String memberNo, Connection conn) throws Exception {
		query = "SELECT A.CART_LIST_NO , A.CART_NO , B.STORE_NAME , B.OPENTIME , B.CLOSETIME , A.MENU_NAME , A.PRICE , SUM(A.QUANTITY) QUANTITY , SUM(A.PRICE*A.QUANTITY) SUBTOTAL FROM ( SELECT C.CART_LIST_NO , C.CART_NO , C.QUANTITY , M.MENU_NAME , M.PRICE FROM CART_LIST C INNER JOIN MENU M ON C.MENU_NO = M.MENU_NO ) A INNER JOIN ( SELECT C.CART_NO , C.USER_NO , S.STORE_NAME , TO_NUMBER(SUBSTR(S.OPENTIME, 1, INSTR(S.OPENTIME, ':')-1)) OPENTIME , TO_NUMBER(SUBSTR(S.CLOSETIME, 1, INSTR(S.CLOSETIME, ':')-1)) CLOSETIME FROM CART C INNER JOIN STORE S ON C.STORE_NO = S.STORE_NO ) B ON A.CART_NO = B.CART_NO LEFT OUTER JOIN ORDERS O ON B.CART_NO = O.CART_NO WHERE O.ORDER_NO IS NULL AND B.USER_NO = ? GROUP BY A.CART_LIST_NO, A.CART_NO, B.STORE_NAME, A.MENU_NAME, A.PRICE , B.OPENTIME, B.CLOSETIME";
